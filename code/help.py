...
from talon import Context, Module, actions, imgui, registry, ui

mod = Module()
mod.list("help_contexts", desc="list of available contexts")
mod.list(
    "help_context_index", desc="available selection numbers for the active help page"
)
selection_numbers = [
    "one",
    "two",
...
def get_current_context_page_length():
    ...
    return len(
        sorted_context_map_keys[start_index : start_index + max_contexts_per_page]
    )
...
def gui_context_help(gui: imgui.GUI):
        ...
        for key in sorted_context_map_keys:
            ...
            if current_context_page == target_page:
                ...
                if (
                    ...
                    and key in cached_active_contexts_list
                ):
    ...
    # if there's a selected context, draw the commands for it
        ...
        if context_map[selected_context] in cached_active_contexts_list:
            gui.text(
                "{} ({}/{}) [ACTIVE]".format(
                    selected_context, selected_context_page, total_page_count
                )
            )
        else:
            gui.text(
            ...
            if selected_context is None:
                selected_context = ctx.lists["self.help_contexts"][
                    sorted_context_map_keys[
                        (current_context_page - 1) * max_contexts_per_page + index
                    ]
                ]
                refresh_help_context_indexes()
